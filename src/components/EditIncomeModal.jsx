...
import { TrendingUp, X, Check, Euro } from 'lucide-react'
import { motion } from 'motion/react'
import { Haptics, ImpactStyle } from '@capacitor/haptics'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'

export default function EditIncomeModal({ month, currentIncome = 0, onSave, onClose }) {
  const [amount, setAmount] = useState(currentIncome > 0 ? String(currentIncome) : '')
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900">Ingresos Totales</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              />
              <Euro className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-emerald-600" />
            </div>
            ...
            <div className="flex items-center gap-1.5 mt-2.5 overflow-x-auto py-0.5 no-scrollbar">
              ...
              {[50, 100, 200, 500].map((extra) => (
                <button
                  key={extra}
                  type="button"
                  onClick={() => handleQuickAdd(extra)}
                  className="text-xs font-semibold px-2.5 py-1 rounded-xl bg-slate-100 hover:bg-emerald-50 hover:text-emerald-700 text-slate-700 border border-slate-200/60 transition-colors whitespace-nowrap active:scale-95"
                >
                  +{extra}€
                </button>
              ))}
          ...
          <p className="text-xs text-slate-500 bg-slate-50 p-3 rounded-xl border border-slate-100 leading-relaxed">
            Esta cantidad se registrará como ingreso total de <strong>{month}</strong> y actualizará la celda de ingresos en tu hoja de cálculo.
          </p>
          ...
          <div className="flex gap-2.5 pt-2">
            ...
            <Button
              ...
              className="flex-1 h-12 rounded-xl text-sm font-semibold bg-emerald-600 hover:bg-emerald-700 text-white shadow-md shadow-emerald-600/25 disabled:opacity-50"
            >
              {submitting ? (
                'Guardando...'
              ) : (
